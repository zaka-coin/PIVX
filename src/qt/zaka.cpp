...
{
    return QCoreApplication::translate("zaka-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in zaka.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in zaka.qrc)
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(zaka_locale);
    Q_INIT_RESOURCE(zaka);
    ...
    /// 6. Determine availability of data directory and parse zaka.conf
    ...
    // zaka: links repeatedly have their payment requests routed to this process:
