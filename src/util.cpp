...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "zaka" is a composite category enabling all PIVX-related debug output
            if (ptrCategory->count(string("zaka"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "zaka.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty zaka.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override zaka.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "zakad.pid"));
